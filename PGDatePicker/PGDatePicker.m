...
- (void)selectedDateLogic {
    ...
    NSString *timeMode = [NSString stringWithFormat:@"%d-%d-%d %d:%d:%d",self.selectedComponents.year,self.selectedComponents.month,self.selectedComponents.day,self.selectedComponents.hour,self.selectedComponents.minute,self.selectedComponents.second];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(datePicker:timeMode:)]) {
        [self.delegate datePicker:self timeMode:timeMode];
    }
}
